...
def imprimir_mapa_asientos(idx_funcion, asientos_map):
    """Imprime el mapa de asientos para una función específica."""
...
def generar_reportes():
    """Calcula y devuelve los reportes estadísticos clave."""
...
def imprimir_usuarios(usuarios_map):
    ...
    for doc, data in usuarios_map.items():
        print(f"DOC: {doc} | Nombre: {data['nombre']} {data['apellido']} | Vínculo: {data['vinculo']}")
    print("-" * 40)
...
def consultar_funciones_cartelera(asientos_map):
    ...
    print(f"Películas en cartelera: {len(peliculas_unicas)} ({', '.join(peliculas_unicas)})")
    print("-" * 40)
